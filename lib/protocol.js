/**
 * 协议相关的处理
 * 协议参照 http://bbs.chinaunix.net/thread-2001015-1-1.html
 * Author: chenboxiang
 * Date: 14-6-15
 * Time: 上午9:36
 */
...
function _parseHeader(headerBuffer, expectedCommand, expectedBodyLength) {
    ...
    if (status !== protocol.HEADER_STATUS_SUCCESS) {
        throw new Error('receive packet errno is: ' + status)
    }

    // validate body length
    var bodyLength = new BigNumber(headerBuffer.toString('hex', 0, protocol.FDFS_PROTO_PKG_LEN_SIZE), 16).toNumber()
    if (null != expectedBodyLength && expectedBodyLength !== bodyLength) {
